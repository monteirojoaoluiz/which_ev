  ...
  return `
    <article class="vehicle-card">
      ...
      <div class="vehicle-card__scores">
        <p>Cost / km <strong>${money.format(metrics.costPerOneStopKm)}</strong></p>
        <p>Cargo + Range <strong>${decimal.format(metrics.cargoRangeScore)}</strong></p>
        <p>Road Trip <strong>${decimal.format(metrics.roadTripBalanceScore)}</strong></p>
        <p>Overall <strong>${decimal.format(metrics.overallScore)}</strong></p>
      </div>
      <p class="vehicle-card__source">
        <a href="${vehicle.sourceUrl}" target="_blank" rel="noreferrer">EV Database</a>
      </p>
    </article>
  `
}
...
app.innerHTML = `
  <main class="page-shell">
    <section class="hero">
      ...
      <h1>Netherlands EV Picks</h1>
      <p>
        Set your target price. We filter
        <strong>±€5,000</strong> around it, then rank by
        value, cargo + range, and road-trip balance.
      </p>
      <p class="hero__meta">Database synced <strong>${updatedAtLabel}</strong></p>
    </section>

    <section class="controls">
      <label for="price-range" class="controls__label">Target Price</label>
    ...
    <section class="leaders" aria-live="polite">
      <article>
        <h2><span class="color-block color-block--red"></span>Value</h2>
        <p>Lowest cost per 1-stop km</p>
        <p id="leader-value" class="leaders__name"></p>
      </article>
      <article>
        <h2><span class="color-block color-block--blue"></span>Utility</h2>
        <p>Cargo + 1-stop range blend</p>
        <p id="leader-cargo-range" class="leaders__name"></p>
      </article>
      <article>
        <h2><span class="color-block color-block--yellow"></span>Road Trip</h2>
        <p>Range, charge speed, value</p>
        <p id="leader-road-trip" class="leaders__name"></p>
      </article>
    </section>

    <section>
      <h2 class="top-three-heading">Top 3</h2>
...
const updateView = (): void => {
  ...
  cargoRangeLeaderEl.textContent = results.metricLeaders.cargoRangeLeader
    ? `${results.metricLeaders.cargoRangeLeader.vehicle.name} — ${decimal.format(results.metricLeaders.cargoRangeLeader.metrics.cargoRangeScore)}`
    : '—'

  roadTripLeaderEl.textContent = results.metricLeaders.roadTripLeader
    ? `${results.metricLeaders.roadTripLeader.vehicle.name} — ${decimal.format(results.metricLeaders.roadTripLeader.metrics.roadTripBalanceScore)}`
    : '—'
